...
class NetworkToolsApp:
    def __init__(self, root):
        try:
            ...
            # Label für Titel                      
            self.title_label = tk.Label(self.tab_speedtest, text="Internet Speedtest", font=("Arial", 16))
            self.title_label.pack(pady=10)
            
            # Button für Speedtest
            self.start_button = tk.Button(self.tab_speedtest, text="Start Speedtest", command=self.start_speedtest)
            self.start_button.pack(pady=10)
            
            # Canvas für Circular Progress
            self.canvas = tk.Canvas(self.tab_speedtest, width=150, height=150, highlightthickness=0)
            ...
            #     label = tk.Label(self.result_frame, text=header, font=("Arial", 10, "bold"), borderwidth=1, relief="solid", width=20)
            #     label.grid(row=0, column=col, padx=2, pady=2)
            #     self.labels[f"header_{col}"] = label

            for row in range(1, 4):  # Für 3 Parameter: Ping, Download, Upload
                for col in range(2):  # Zwei Spalten: Parameter und Wert
                    anchor = "e" if col == 0 else "w"
                    label = tk.Label(self.result_frame, text="", font=self.font16bold, borderwidth=0, relief="solid", width=20)
                    label.grid(row=row, column=col, padx=2, pady=2)
                    self.labels[f"row{row}_col{col}"] = label    
            ##############################################################
    ...
    def animate_spinner(self):
        ...
        extent = self.angle
        self.canvas.create_arc(10, 10, 140, 140, start=self.start_angle, extent=extent, outline="blue", style="arc", width=15)
        
        # Aktualisiere den Winkel
        self.angle = (self.angle + 10) % 360
        
        # Animation fortsetzen
        if self.start_button["state"] == "disabled":  # Nur animieren, wenn der Test läuft
